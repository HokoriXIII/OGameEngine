            ...
            foreach (String line in lines)
            {
                ...
                else if (line.StartsWith("f ")) // Face definition
                {
                    ...
                    if (temp.Trim().Count((char c) => c == ' ') == 2) // Check if there's enough elements for a face
                    {
                        ...
                        bool success = int.TryParse(faceparts[0].Split('/')[0], NumberStyles.Integer, CultureInfo.InvariantCulture, out v1);
                        success |= int.TryParse(faceparts[1].Split('/')[0], NumberStyles.Integer, CultureInfo.InvariantCulture, out v2);
                        success |= int.TryParse(faceparts[2].Split('/')[0], NumberStyles.Integer, CultureInfo.InvariantCulture, out v3);

                        if (faceparts[0].Count((char c) => c == '/') >= 2)
                        {
                            success |= int.TryParse(faceparts[0].Split('/')[1], NumberStyles.Integer, CultureInfo.InvariantCulture, out t1);
                            success |= int.TryParse(faceparts[1].Split('/')[1], NumberStyles.Integer, CultureInfo.InvariantCulture, out t2);
                            success |= int.TryParse(faceparts[2].Split('/')[1], NumberStyles.Integer, CultureInfo.InvariantCulture, out t3);
                            success |= int.TryParse(faceparts[0].Split('/')[2], NumberStyles.Integer, CultureInfo.InvariantCulture, out n1);
                            success |= int.TryParse(faceparts[1].Split('/')[2], NumberStyles.Integer, CultureInfo.InvariantCulture, out n2);
                            success |= int.TryParse(faceparts[2].Split('/')[2], NumberStyles.Integer, CultureInfo.InvariantCulture, out n3);
                        }
                        else
                        {
